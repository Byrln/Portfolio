...
export default function ProjectGrid({ initialProjects = projects }: ProjectGridProps) {
  const [filter, setFilter] = useState<string>("all");
  const [visibleCount, setVisibleCount] = useState<number>(3); // Show 3 rows

  const categories = [
    { id: "all", name: "Mobile Apps" },
        ...
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              className={`px-5 py-2.5 rounded-lg font-medium text-sm transition-all duration-300 relative ${isActive ? "text-gray-900" : "text-gray-300 hover:text-white"}`}
            >
              ...
              {isActive && <span className="ml-2 relative z-10 bg-gray-900/30 text-white text-xs px-2 py-0.5 rounded-full">{filteredProjects.length}</span>}
            </button>
          );
        })}
      </div>
